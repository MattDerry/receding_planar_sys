...
TASK_COLLISION_TIME_PENALTY = 10.0
ERROR_SCALING = 450.0

OBSTACLE_WIDTH_LIMITS = [0.05, 0.4]
OBSTACLE_HEIGHT_LIMITS = [0.3, 1.0]
OBSTACLE_XPOS_LIMITS = [[-0.8,-0.5],[-0.3,0.3],[0.5,0.8]]

PREACTIVE_TIME = rospy.Duration(1.0)
MAX_OBSTACLE_LIFETIME = 4.0
MIN_OBSTACLE_LIFETIME = 1.0

TARGET_X_LIMITS = [[-0.975,-0.025],[0.025,0.975]]
TARGET_Y_LIMITS = [-0.4,-0.05]
TARGET_RADIUS = 0.1
...
def makeTargetMarker():
        ...
        marker.scale.x = TARGET_RADIUS
        marker.scale.y = TARGET_RADIUS
        marker.scale.z = 0.01
        marker.color.r = 0.0
        marker.color.g = 0.0
        marker.color.b = 1.0
...
class TaskStateMachine:
    def __init__(self, targets, left_targets, right_targets):
        ...
        self.in_obst = False
        self.active_obstacle_ids = []
        self.all_obstacle_ids = [0, 1, 2]
        self.available_obstacle_ids = self.all_obstacle_ids
        self.IN_TARGETS_THRESHOLD = 20
        if random.randint(0, 1) == 1:
            self.LEFT_TARGET = True
        else:
    ...
    def updateState(self, mass_pos):
        if self.current_state == self.states.READY:
            if self.target_count > 1:
                ...
                if self.LEFT_TARGET:
                    self.create_target(TARGET_X_LIMITS[1])
                else:
                    self.create_target(TARGET_X_LIMITS[0])
                self.LEFT_TARGET = not self.LEFT_TARGET
    ...
    def check_and_deactivate_expired_obstacles(self):
        for obstacle in obstacle_markers:
            if obstacle.state == obstacle.possible_states.ACTIVE and obstacle.activation_time + obstacle.marker.lifetime < rospy.Time.now():
                obstacle.deactivate()
                self.available_obstacle_ids.append(obstacle.marker.id)

    def count_active_obstacles(self):
        count = 0
        for obstacle in obstacle_markers:
            if obstacle.state == obstacle.possible_states.ACTIVE or obstacle.state == obstacle.possible_states.PREACTIVE:
                count += 1
        return count


    def preactivate_obstacle(self, oid):
        for obstacle in obstacle_markers:
            if obstacle.marker.id == oid:
                obstacle.randomize(OBSTACLE_WIDTH_LIMITS, OBSTACLE_HEIGHT_LIMITS, OBSTACLE_XPOS_LIMITS[oid])
                while obstacle.target_intersection(self.current_target_marker):
                    obstacle.randomize(OBSTACLE_WIDTH_LIMITS, OBSTACLE_HEIGHT_LIMITS, OBSTACLE_XPOS_LIMITS[oid])
                obstacle.preactivate()
                self.obstacle_pub.publish(obstacle.marker)

    def activate_obstacles(self):
        for obstacle in obstacle_markers:
            if obstacle.state == obstacle.possible_states.PREACTIVE and obstacle.preactivation_time + PREACTIVE_TIME < rospy.Time.now():
                lifetime = random.random() * (MAX_OBSTACLE_LIFETIME - MIN_OBSTACLE_LIFETIME) + MIN_OBSTACLE_LIFETIME
                obstacle.activate(lifetime)
                self.obstacle_pub.publish(obstacle.marker)
    ...
    def create_target(self, target_x_limits):
        self.current_target_marker.pose.position.x = (target_x_limits[1] - target_x_limits[0]) * random.random() + target_x_limits[0]
        self.current_target_marker.pose.position.y = (TARGET_Y_LIMITS[1] - TARGET_Y_LIMITS[0]) * random.random() + TARGET_Y_LIMITS[0]
        while self.target_in_obstacle(self.current_target_marker):
            self.current_target_marker.pose.position.x = (target_x_limits[1] - target_x_limits[0]) * random.random() + target_x_limits[0]
            self.current_target_marker.pose.position.y = (TARGET_Y_LIMITS[1] - TARGET_Y_LIMITS[0]) * random.random() + TARGET_Y_LIMITS[0]
        self.current_target_marker.header.stamp = rospy.Time.now()

    def in_current_target(self, mass_pos):
        dist = self.distance(mass_pos.xm, mass_pos.ym, self.current_target_marker.pose.position.x, self.current_target_marker.pose.position.y)
    ...
    def in_obstacle(self, mass_pos):
        for obstacle in obstacle_markers:
            if obstacle.state == obstacle.possible_states.ACTIVE:
                xmin = obstacle.marker.pose.position.x - (obstacle.marker.scale.x/2 + 0.025)
                xmax = xmin + obstacle.marker.scale.x + 0.05
                ymin = obstacle.marker.pose.position.y - (obstacle.marker.scale.y/2 + 0.025)
                ymax = ymin + obstacle.marker.scale.y + 0.05
                if self.in_range(mass_pos, xmin, xmax, ymin, ymax):
                    self.collision_marker.pose.position.x = obstacle.marker.pose.position.x
                    self.collision_marker.pose.position.y = obstacle.marker.pose.position.y
                    self.collision_marker.scale.x = obstacle.marker.scale.x
                    self.collision_marker.scale.y = obstacle.marker.scale.y
    ...
    if rospy.has_param("xlim") and rospy.has_param("ylim"):
        ...
        if rospy.has_param("obstacles"):
            ...
            for obstacle in obstacles:
                ...
                obstacle_markers.append(Obstacle(obstacle, o_id))
                o_id += 1
