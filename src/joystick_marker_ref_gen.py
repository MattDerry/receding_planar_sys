...
class MarkerControls:
    def __init__(self):
        ...
        self.conf_serv = rospy.Service("get_user_control_params", UserControlParams, self.control_params_handler)
    ...
    def control_params_handler(self, req):
        return {'trust_estimate' : trial_trust, 'cutoff_frequency' : CUTOFF, 'alpha' : ALPHA}
...
def main():
    ...
    if rospy.has_param("user_trust_history"):
        ...
        count = 0.
        trust_sum = 1.0
        for trust in trust_history:
            trust_sum = ((1-ALPHA)*trust_sum) + (ALPHA*trust)
        trial_trust = trust_sum
    else:
